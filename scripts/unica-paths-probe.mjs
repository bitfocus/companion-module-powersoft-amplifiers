...
if (!host || !portRaw) {
  console.error('Usage: node scripts/unica-paths-probe.mjs <host> <port> [--https] [--auto-channels|--channels N] [--csv file] [--out file]')
  process.exit(1)
}
...
function substitute(p, i0, i1) {
  return p
    .replace(/\{0(?::\d+)?\}/g, String(i0))
    .replace(/\{1(?::\d+)?\}/g, String(i1))
}
...
async function main() {
  ...
  if (flagz.out) {
    const report = {
      meta: { host, port, https: useHttps, channels, csv: path.relative(process.cwd(), csvPath), timestamp: new Date().toISOString() },
      results,
    }
    try {
